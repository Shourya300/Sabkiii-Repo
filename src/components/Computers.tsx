...
// Fix for color management if needed, though usually handled by Canvas
...
export function Instances({ children, ...props }: InstancesProps) {
    const { nodes } = useGLTF('/nsponsor/computers_1-transformed.glb') as any
    const instances = useMemo(
        () => {
            if (!nodes) return {} as any;
            return {
    ...
    return (
        <Merged castShadow receiveShadow meshes={instances} {...props}>
            {(instances: any) => <context.Provider value={instances}>{children}</context.Provider>}
        </Merged>
    )
}
...
const SCREENS = [
    { frame: "Object_206", panel: "Object_207", position: [0.27, 1.53, -2.61], rotation: [0, 0, 0] },
    { frame: "Object_209", panel: "Object_210", position: [-1.43, 2.5, -1.8], rotation: [0, 1, 0], y: 5 },
    { frame: "Object_212", panel: "Object_213", position: [-2.73, 0.63, -0.52], rotation: [0, 1.09, 0], x: -5, y: 5, invert: true },
    { frame: "Object_215", panel: "Object_216", position: [1.84, 0.38, -1.77], rotation: [0, -Math.PI / 9, 0], invert: true },
    { frame: "Object_218", panel: "Object_219", position: [3.11, 2.15, -0.18], rotation: [0, -0.79, 0], x: -5, scale: 0.81, invert: true },
    { frame: "Object_221", panel: "Object_222", position: [-3.42, 3.06, 1.3], rotation: [0, 1.22, 0], y: 5, scale: 0.9 },
    { frame: "Object_224", panel: "Object_225", position: [-3.9, 4.29, -2.64], rotation: [0, 0.54, 0], invert: true },
    { frame: "Object_227", panel: "Object_228", position: [0.96, 4.28, -4.2], rotation: [0, -0.65, 0] },
    { frame: "Object_230", panel: "Object_231", position: [4.68, 4.29, -1.56], rotation: [0, -Math.PI / 3, 0] },
];
...
export function Computers({ sponsors = [], ...props }: ComputersProps) {
    const { nodes: n, materials: m } = useGLTF('/nsponsor/computers_1-transformed.glb') as any
    const instances = useContext(context)

    // Early return if instances are not yet available to avoid 'undefined' errors
    if (!instances || !n) return null;
    ...
    return (
        <group {...props} dispose={null}>
            ...
            {/* Dynamic Screens with Logos */}
            {sponsors.length > 0 && SCREENS.map((screen: any, index: number) => {
...
function Screen({ frame, panel, children, ...props }: any) {
    const { nodes, materials } = useGLTF('/nsponsor/computers_1-transformed.glb') as any
    return (
        <group {...props}>
...
function ScreenLogo({ text, invert, ...props }: any) {
    return (
        <Screen {...props}>
            <PerspectiveCamera makeDefault manual aspect={1 / 1} position={[0, 0, 15]} />
            <color attach="background" args={[invert ? 'black' : '#E62B1E']} />
            <ambientLight intensity={1} />
            <directionalLight position={[10, 10, 5]} intensity={2} />
            <TextPlane text={text} invert={invert} />
        </Screen>
    )
}

function TextPlane({ text, invert }: { text: string, invert: boolean }) {
    const textRef = useRef<any>(null)

    useFrame((state) => {
        if (textRef.current) {
            // Oscillate left-right: Range of 1.5 units, speed of 0.8
            textRef.current.position.x = Math.sin(state.clock.elapsedTime * 0.8) * 1.5
        }
    })
    ...
    return (
        <Text
            ref={textRef}
            position={[0, 1, 0]} // Shifted up by 2 units
            fontSize={1.5} // Reduced from 1.4 for fitting
            maxWidth={10}   // Increased max width to avoid wrapping
            lineHeight={1}
            letterSpacing={-0.05}
            textAlign="center"
            // Using a standard font, or we can use the default
            font="https://fonts.gstatic.com/s/inter/v12/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuLyfAZ9hjp-Ek-_EeA.woff"
            color={invert ? '#E62B1E' : 'white'} // If screen bg is black (invert), text is red. If bg is red, text is white.
            anchorX="center"
            anchorY="middle"
        >
            {text}
    ...
    useFrame((state) => {
        if (ref.current) {
            ref.current.children.forEach((instance: any) => {
                ...
                // Set LED color to TedX Red variation
                instance.color.setRGB(0.9, t * 0.1, t * 0.1) // Reddish
            })
        }
    })
    return (
        <group ref={ref}>
            <instances.Sphere position={[-0.41, 1.1, -2.21]} scale={0.005} color={[1, 0, 0]} />
            <instances.Sphere position={[0.59, 1.32, -2.22]} scale={0.005} color={[1, 0, 0]} />
            <instances.Sphere position={[1.77, 1.91, -1.17]} scale={0.005} color={[1, 0, 0]} />
            <instances.Sphere position={[2.44, 1.1, -0.79]} scale={0.005} color={[1, 0, 0]} />
            <instances.Sphere position={[4.87, 3.8, -0.1]} scale={0.005} color={[1, 0, 0]} />
            <instances.Sphere position={[1.93, 3.8, -3.69]} scale={0.005} color={[1, 0, 0]} />
            <instances.Sphere position={[-2.35, 3.8, -3.48]} scale={0.005} color={[1, 0, 0]} />
            <instances.Sphere position={[-4.71, 4.59, -1.81]} scale={0.005} color={[1, 0, 0]} />
            <instances.Sphere position={[-3.03, 2.85, 1.19]} scale={0.005} color={[1, 0, 0]} />
